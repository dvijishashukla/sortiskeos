...
function ActionPill({ action }) {
  const style = ACTION_COLORS[action] || { bg: "rgba(100,116,139,0.15)", color: "#94a3b8", border: "#64748b" };
  return (
    <span style={{
      background: style.bg,
      color: style.color,
    ...
    <>
      ...
      <div style={{
        ...
        {loading ? (
          ...
          entries.map((entry, i) => {
            const isCritical = entry.action === "tamper_detected" || entry.action === "antiforensics_detected";
            return (
              <div
                key={i}
                className="table-row"
